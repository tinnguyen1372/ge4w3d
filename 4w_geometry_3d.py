...
def create_geometry_3d(cube_size, air_size, wall_thickness):
    ...
    geometry = np.zeros((cube_size, cube_size,100), dtype=int)
...
def add_random_shape_3d(i, geometry, air_start, air_end, wall_thickness, cube_size=200):
    ...
    center_z = int(z_max * 0.5)  # Middle along z-axis

    # Adjust start positions to be closer to the center while avoiding out-of-bounds errors
    x_start = random.randint(
        max(air_start + wall_thickness + objwall_gap, 0), 
        min(air_end - size_w - cube_size // 4, x_max - size_w)
    )
    y_start = random.randint(
        max(air_start + int(7*cube_size/22), 0), 
        min(air_end - size_h - int(7*cube_size/22), y_max - size_h)
    )
